...
def convert_filter_to_df(filter_dict):
    ...
    for field in direct_filters:
        if filter_dict.get(field):
            filtered_df = filtered_df[filtered_df[field].isin(filter_dict[field])]

    final_df = filtered_df.drop_duplicates(subset='BEIS School ID')

    # Reverse column renaming for consistency with backend
